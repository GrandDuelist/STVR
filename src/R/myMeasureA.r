...
#  versions <- c("litmus_30","litmus_35","litmus_36","litmus_40");
  ...
  results = list();
  output_result = matrix(ncol = 13, nrow = 14);
  names = c("random","TextDiversity","TopicDiversity","RiskDrivenRandom","RiskDrivenDiverse","measureA(RDD&R)",
            "measureA(RDD&TD)","measureA(RDD&TC)","measureA(RDD&RR)","p(RDD&R)", "p(RDD&TD)","p(RDD&TC)","p(RDD&RR)");
  colnames(output_result) <- names;
  rownames(output_result) <-versions[1:14];
  
 
  for(i in 1:14){
    version = versions[i];
    result <- read.xls(fileDir,sheet=i);
...
myMeasureATablets <-function()
{
  #versions <- c("litmus_50",
  #"litmus_60","litmus_70","litmus_80","litmus_90","litmus_10",
  #"litmus_11","litmus_12","litmus_13");
  fileDir = "/home/zhihan/Workspace/STVR/data/RESULT/Mobile Firefox Tablets Only/tablets_result.xlsx";
  versions = sheetNames(fileDir)
